...
if (Get-Command Set-PSReadLineOption -ErrorAction SilentlyContinue) {
    ...
    # The built-in word movement uses character delimiters, but token based word movement is also very useful
    ...
    # Cycle through quote styles on current token
    Set-PSReadLineKeyHandler -Key "Alt+'" `
        ...
        foreach ($token in $tokens) {
            ...
            if ($extent.StartOffset -le $cursor -and $extent.EndOffset -ge $cursor) {
                ...
                # If the cursor is at the end of the previous token, we only want to change the previous token if there is no token under the cursor
    ...
    # Replace aliases on command line with full commands
    Set-PSReadLineKeyHandler -Key "Alt+%" `
    ...
    # Directory Marks Jumping system
    ...
    # Make RightArrow accept the next word in prediction suggestion instead of whole line
    ...
    # Cycle/Select Command Arguments
